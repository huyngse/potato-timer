// Thời kỳ hậu chiến và bao cấp (1975-1986)

const GROUP_ID = 4;

export const ep4 = [
    {
        id: "ep4-2",
        content:
            "Hội nghị hiệp thương chính trị Bắc - Nam thống nhất đất nước về mặt nhà nước<br>(11/1975)",
        start: "1975-11-15",
        group: GROUP_ID,
    },
    {
        id: "ep4-3",
        content:
            "Tổ chức Tổng tuyển cử bầu Quốc hội chung cho cả nước<br>(25/4/1976)",
        start: "1976-04-25",
        group: GROUP_ID,
    },
    {
        id: "ep4-4",
        content:
            "Thực hiện thống nhất đất nước về mặt nhà nước:<br> thành lập nước Cộng hòa Xã hội Chủ nghĩa Việt Nam<br>(2/7/1976)",
        start: "1976-07-02",
        group: GROUP_ID,
    },
    {
        id: "ep4-5",
        content:
            "Quân tình nguyện Việt Nam giúp Campuchia lật đổ chế độ diệt chủng Pol Pot<br>(1/1979)",
        start: "1979-01-07",
        group: GROUP_ID,
    },
    {
        id: "ep4-6",
        content:
            "Chiến tranh biên giới phía Bắc, quân Trung Quốc xâm lược Việt Nam<br>(17/2/1979)",
        start: "1979-02-17",
        group: GROUP_ID,
    },
    {
        id: "ep4-7",
        content:
            "Bắt đầu các hiện tượng 'xé rào', 'khoán chui'<br> trong cải cách nông nghiệp ở miền Bắc<br>(cuối năm 1979)",
        start: "1979-12-01",
        group: GROUP_ID,
    },
    {
        id: "ep4-8",
        content:
            "Khủng hoảng kinh tế - xã hội, lạm phát tăng nhanh,<br> đời sống nhân dân khó khăn nghiêm trọng<br>(1980-1986)",
        start: "1980-01-01",
        group: GROUP_ID,
    },
    {
        id: "ep4-9",
        content: "Tổng Bí thư Lê Duẩn qua đời<br>(10/7/1986)",
        start: "1986-07-10",
        group: GROUP_ID,
    },
    {
        id: "ep4-10",
        content:
            "Đồng chí Trường Chinh được bầu làm Tổng Bí thư, chuẩn bị Đại hội VI<br>(14/7/1986)",
        start: "1986-07-14",
        group: GROUP_ID,
    },
];